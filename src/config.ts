export const PAGE_SIZE = 15;
export const USER_INPUT_DEBOUNCE = 500;
export const LONG_CLICK_TIME = 0.5;
export const COLORS = ['magenta', 'red', 'volcano', 'orange', 'gold', 'lime', 'green', 'cyan', 'blue', 'geekblue', 'purple'];

const { protocol, hostname } = window.location;
export const AUTH_SERVER = `${protocol}//${hostname}:8080/auth`;
export const API_SERVER = `${protocol}//${hostname}:8080/api`;
export const FS_SERVER = `${protocol}//${hostname}:8080/fs`;

export const SOCKET_SERVER = `${protocol}//${hostname}:8080`;
export const FFMPEG_SOCKET_PATH = '/api/socket.io/ffmpeg';
export const VIDEO_EXAMINE_SOCKET_PATH = '/api/socket.io/videos/examine';
export const MUSIC_EXAMINE_SOCKET_PATH = '/api/socket.io/musics/examine';
